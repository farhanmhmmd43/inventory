...
class Aksesoris extends Component {
  ...
  get_listview = (state, instance, customFilter = []) => {
    ...
    axios({
      ...
      url: 'api/v1/cms/get-all-prospect-acc',
      params,
      headers: {
      ...
      this.props.processResponse(response).then((result) => {
        ...
        result.data.data.lists.map((v) => {
          listview_data.push({
            no,
            id: v.id,
            product_variant_id: v.product_acc ? v.product_acc.product_variant ? v.product_acc.product_variant.id : '' : '',
            nama_customer: v.name,
            no_telp: v.phone_number,
            kota: v.city ? v.city.city_name : '',
            kecamatan: v.kecamatan,
            aksesoris: v.product_acc ? v.product_acc.acc_name : '',
            maindealer: v.maindealer ? v.maindealer.name : '',
            konfirmasi_aksesoris: v.product_acc_confirm ? v.product_acc_confirm.acc_name : '',
  ...
  get_detail = (id) => {
    ...
    axios({
      method: 'POST',
      baseURL: this.props.variables.api_url,
      url: 'api/v1/cms/get-prospect-acc-detail',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.props.userData.access_token
      },
      ...
      this.props.processResponse(response).then((result) => {
        const form_data = {
          ...
          product_acc_id: result.data.data.product_acc ? result.data.data.product_acc.id : '',
          product_acc_id_confirm: result.data.data.product_acc_confirm ? result.data.data.product_acc_confirm.id : '',
  ...
  update = (data) => {
    ...
    axios({
      method: 'POST',
      baseURL: this.props.variables.api_url,
      url: 'api/v1/cms/update-prospect-acc',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.props.userData.access_token
      },
  ...
  get_note = (prospect_id) => axios({
    method: 'POST',
    baseURL: this.props.variables.api_url,
    url: 'api/v1/cms/get-prospect-acc-notes',
    headers: {
      "Content-Type": "application/json",
      "Authorization": "Bearer " + this.props.userData.access_token
    },
  ...
  update_note = (data) => {
    ...
    axios({
      method: 'POST',
      baseURL: this.props.variables.api_url,
      url: 'api/v1/cms/update-prospect-acc-notes',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.props.userData.access_token
      },
  ...
  get_all_acc = (product_variant_id) => axios({
    method: 'GET',
    baseURL: this.props.variables.api_url,
    ...
    this.props.processResponse(response).then((result) => {
      let acc_options = [{}];
      result.data.data.map((v) => {
        acc_options.push({ name: v.acc_name, value: v.id });
        return true;
      });
              ...
              <CardBody>
                <CustomTable
                  ...
                  buttons={[
                    {
                      type: 'download',
                      url: 'api/v1/cms/download-prospect-acc',
                      name: 'Prospect Aksesoris',
                      access: [1, 2]
                    }
                  ]}
                  action={[
                    {
                      ...
                      onClick: (e) => {
                        ...
                        Promise.all([
                          ...
                          this.get_all_city(),
                          this.get_all_md(),
                          this.get_all_acc(e.product_variant_id)
                        ]).then(() => {
                          this.setState({
                            modal_edit: true,
                            loading: false
                  ...
                  columns={[
                    {
                      ...
                      Header: 'Konfirmasi Aksesoris',
                      accessor: 'konfirmasi_aksesoris'
                    }, {
                      Header: 'Tanggal',
                      accessor: 'tanggal'
                    }, {
        ...
        <Edit
          ...
          inputs={[
            ...
            },
            {
              ...
              label: 'Aksesoris',
              col: 6,
              options: this.state.acc_options
            },
            {
              type: 'selectsearch',
              value: this.state.form_data.product_acc_id_confirm,
              name: 'product_acc_id_confirm',
              label: 'Konfirmasi Aksesoris',
              col: 6,
              options: this.state.acc_options
            },
            {
