...
class SukuCadang extends Component {
  ...
  get_listview = (state, instance, customFilter = []) => {
    ...
    axios({
      ...
      url: 'api/v1/cms/get-all-prospect-part',
      params,
      headers: {
      ...
      this.props.processResponse(response).then((result) => {
        ...
        result.data.data.lists.map((v) => {
          listview_data.push({
            no,
            id: v.id,
            product_variant_id: v.product_part ? v.product_part.product_variant ? v.product_part.product_variant.id : '' : '',
            nama_customer: v.name,
            no_telp: v.phone_number,
            kota: v.city ? v.city.city_name : '',
            kecamatan: v.kecamatan,
            suku_cadang: v.product_part ? v.product_part.part_name : '',
            konfirmasi_suku_cadang: v.product_part_confirm ? v.product_part_confirm.part_name : '',
  ...
  get_detail = (id) => axios({
    method: 'POST',
    baseURL: this.props.variables.api_url,
    url: 'api/v1/cms/get-prospect-part-detail',
    headers: {
      "Content-Type": "application/json",
      "Authorization": "Bearer " + this.props.userData.access_token
    },
    ...
    this.props.processResponse(response).then((result) => {
      const form_data = {
        ...
        product_part_id: result.data.data.product_part ? result.data.data.product_part.id : '',
        product_part_id_confirm: result.data.data.product_part_confirm ? result.data.data.product_part_confirm.id : '',
  ...
  update = (data) => {
    ...
    axios({
      method: 'POST',
      baseURL: this.props.variables.api_url,
      url: 'api/v1/cms/update-prospect-part',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.props.userData.access_token
      },
  ...
  get_note = (prospect_id) => axios({
    method: 'POST',
    baseURL: this.props.variables.api_url,
    url: 'api/v1/cms/get-prospect-part-notes',
    headers: {
      "Content-Type": "application/json",
      "Authorization": "Bearer " + this.props.userData.access_token
    },
  ...
  update_note = (data) => {
    ...
    axios({
      method: 'POST',
      baseURL: this.props.variables.api_url,
      url: 'api/v1/cms/update-prospect-part-notes',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.props.userData.access_token
      },
  ...
  get_all_part = (product_variant_id) => axios({
    method: 'GET',
    baseURL: this.props.variables.api_url,
    ...
    this.props.processResponse(response).then((result) => {
      ...
      result.data.data.map((v) => {
        part_options.push({ name: v.part_name, value: v.id });
        return true;
      });
              ...
              <CardBody>
                <CustomTable
                  ...
                  buttons={[
                    {
                      type: 'download',
                      url: 'api/v1/cms/download-prospect-part',
                      name: 'Prospect Suku Cadang',
                      access: [1, 2]
                    }
                  ]}
                  action={[
                    {
                      ...
                      onClick: (e) => {
                        ...
                        Promise.all([
                          ...
                          this.get_all_city(),
                          this.get_all_part(e.product_variant_id)
                        ]).then(() => {
                          this.setState({
                            modal_edit: true,
                            loading: false
                  ...
                  columns={[
                    {
                      ...
                      Header: 'Konfirmasi Suku Cadang',
                      accessor: 'konfirmasi_suku_cadang'
                    }, {
                      Header: 'Tanggal',
                      accessor: 'tanggal'
                    }, {
        ...
        <Edit
          ...
          inputs={[
            ...
            },
            {
              ...
              label: 'Suku Cadang',
              col: 6,
              options: this.state.part_options
            },
            {
              type: 'selectsearch',
              value: this.state.form_data.product_part_id_confirm,
              name: 'product_part_id_confirm',
              label: 'Konfirmasi Suku Cadang',
              col: 6,
              options: this.state.part_options
            },
            {
